...
class UserController {
    ...
    async getById(data) {
        try {
            const sql = `SELECT * FROM ${process.env.DATABASE_USERS_TABLE} WHERE id_users = ?`;
            const [results] = await database.promise().query(sql, [data]);
            if(!results.length) {
                return false;
            }
            const user = results[0];
    ...
    async delete(email) {
        try {
            ...
            const sql2 = `DELETE FROM ${process.env.DATABASE_USERS_TABLE} WHERE email = ?`;
            await database.promise().query(sql2, [email]);
        } catch (error) {
            console.log('Error in updateHobbies function of UserController: ', error);
        }
    }
