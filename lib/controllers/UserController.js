...
import uploadUserImageProfile from "../middlewares/multer";
...
class UserController {
    ...
    async updateProfileImage(email, file) {
        try {
            ...
            console.log(file);
            uploadUserImageProfile(dateNow).single(file);
            const fileName = dateNow + path.extname(file.originalname);
            const sql = `UPDATE ${process.env.DATABASE_USERS_TABLE} SET profile_image_path = ? WHERE email = ?`;
            await database.promise().query(sql, [`/users-profiles/${fileName}`, email]);
        } catch (error) {
